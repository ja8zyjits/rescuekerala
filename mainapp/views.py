...
def relief_camps_list(request):
    ...
    relief_camps = filter.qs.annotate(count=Count(
        'person')).order_by('district', 'name').all()
...
def districtmanager_list(request):
    filter = DistrictManagerFilter(
        request.GET, queryset=DistrictManager.objects.all())
...
def mapdata(request):
    ...
    if district != "all":
        data = Request.objects.exclude(
            latlng__exact="").filter(district=district).values()
    else:
...
class PeopleFilter(django_filters.FilterSet):
    fields = ['name', 'phone', 'address',
              'district', 'notes', 'gender', 'camped_at']
...
def announcements(request):
    link_data = Announcements.objects.filter(
        is_pinned=False).order_by('-id').all()
    pinned_data = Announcements.objects.filter(
        is_pinned=True).order_by('-id').all()[:5]
...
def coordinator_home(request):
    filter = CoordinatorCampFilter(
        request.GET, queryset=RescueCamp.objects.all())
    data = filter.qs.annotate(count=Count('person')).order_by(
        'district', 'name').all()
